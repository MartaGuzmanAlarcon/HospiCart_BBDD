...
//            		break;
	...
	//TODO CHECK WHAT HAPPENS IF WE INTRODUCE AN INVALID NUMBER
...
//	}
	 ...
	 */
	 public void browseAllProducts() {
		 List<Product> products = productManager.getAllProducts();
		 //The nurse can only buy products that do not need prescription and that is what we will show when he/she selects "browse all products"
		 List<Product> productsWithoutPrescription = new ArrayList<>();
		 for(int i = 0; i < products.size(); i++) {
			 if(products.get(i).getNeedPrescription() == false) {
				 productsWithoutPrescription.add(products.get(i));
			 }
		 }
		 for(int i = 0; i < productsWithoutPrescription.size(); i++) {
			 System.out.println(productsWithoutPrescription.get(i));
		 }
	 } 
	 ...
	 public void accountSettings() throws SQLException{
	    ...
	    while(keepGoing) {	    	
		    ...
		    case 1: 
		    	...
		    	keepGoing = false;
		   		displayNurseMenu(); //I take the supplier back to the supplier menu
		   		break;
		    case 2:
